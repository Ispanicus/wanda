...
def get_wikipedia(nsamples, seed, seqlen, tokenizer):
    ...
    for i, url in enumerate(traindata['url']):
        if url in urls:
            indices.append(i)
    ...
    # Encode datasets
    trainenc = tokenizer(" ".join(filtered_dataset['text'][:-128]), return_tensors='pt')
    testenc = tokenizer("\n\n".join(filtered_dataset['text'][-128:]), return_tensors='pt')
...
def get_loaders(name, nsamples=4, seed=0, seqlen=2048, tokenizer=None):
