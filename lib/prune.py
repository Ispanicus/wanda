...
def check_sparsity(model):
    ...
    if "bloom" in model.name_or_path:
...
def prepare_calibration_input(model, dataloader, device):
    ...
    cache = {'i': 0, 'attention_mask': None, position: None}
    ...
    class Catcher(nn.Module):
        ...
        def forward(self, inp, **kwargs):
            ...
            cache[position] = kwargs[position]
            raise ValueError
    ...
    position_ids = cache[position]
    model.config.use_cache = use_cache

    if 'bloom' in model.name_or_path:
        return inps, outs, position_ids, attention_mask
    else:
...
def prune_wanda(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    ...
    if "bloom" in args.model:
        with torch.no_grad():
            inps, outs, position_ids, attention_mask = prepare_calibration_input(model, dataloader, device)
        layers = next((m for m in model.modules() if isinstance(m, torch.nn.ModuleList)), None)
        for i, layer in enumerate(tqdm(layers)):
            subset = find_layers(layer)
            ...
            for j in range(args.nsamples):
                with torch.no_grad():
                    outs[j] = layer(inps[j].unsqueeze(0), alibi=position_ids, attention_mask=attention_mask)[0]
            for h in handles:
                h.remove()
